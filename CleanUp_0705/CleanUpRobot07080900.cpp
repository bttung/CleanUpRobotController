...
double MyController::onAction(ActionEvent &evt)
{
	...
	switch(m_state) {
		...
		case 20: {
			// 送られた座標に回転する
			m_time = rotateTowardObj(nextPos, m_vel / 5, evt.time());
			//printf("debug %lf %lf \n", evt.time(), m_time);
			m_state = 21;
			m_executed = false;
			break;
		...
		case 730: {
			// 送られた座標に回転する
			m_time = rotateTowardObj(m_tpos, m_vel / 5, evt.time());
		...
		case 740: {
			...
			if(m_nodeId < m_route.size()) {
				...
				//m_trashBoxPos.x(), m_trashBoxPos.z()
				m_time = rotateTowardObj(m_trashBoxPos, m_vel / 5, evt.time());
	...
	case 750: {
		...
		case 810: {
			...
			if(evt.time() > m_time && m_executed == false) {
				m_my->setWheelVelocity(0.0, 0.0);
				m_time = rotateTowardObj(m_lookingPos, m_vel / 5, evt.time());
				m_executed = false;
				m_state = 815;
			}
			break;
