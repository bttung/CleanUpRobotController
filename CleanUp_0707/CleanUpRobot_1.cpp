...
double MyController::onAction(ActionEvent &evt)
{
	...
	case 750: {
		...
		case 810: {
			...
			if(evt.time() > m_time && m_executed == false) {
				m_my->setWheelVelocity(0.0, 0.0);
				setRobotHeadingAngle(m_requiredHeadingAngle);
				printf("robot rotated \n");
				m_time = evt.time() + 1.0;
				m_executed = true;
				m_state = 815;
			}
		...
		case 815: {
			// 送られた座標に移動中
			if(evt.time() > m_time && m_executed == true) {
				sendSceneInfo();
  ...
  if(sender == "RecogTrash"){
	 ...
	 if(strcmp(header, "ThrowTrash") == 0) {			
		...
		if(strcmp(header, "RandomRoute") == 0) {
			...
			double headingX = atof(strtok_r(NULL, delim, &ctx));
			double headingZ = atof(strtok_r(NULL, delim, &ctx));
			// calcRequiredHeadingangle
			double diffX = x - headingX;
			double diffZ = z - headingZ;
			m_requiredHeadingAngle = atan(diffX / diffZ) * 180 / PI;
			printf("diffX: %lf diffZ: %lf requireAngle: %lf \n", diffX, diffZ, m_requiredHeadingAngle); 			
			if (diffZ >= 0) {
				m_requiredHeadingAngle += 180;
			}
